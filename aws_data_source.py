...
def _lookup_mitre(finding_type):
    """Look up MITRE ATT&CK technique and tactic for a finding type."""
    result = MITRE_MAPPING.get(finding_type)
    if result:
        return result[0], result[2]
    if finding_type.startswith("CloudTrail-"):
        event_name = finding_type.replace("CloudTrail-", "")
        ct_result = CLOUDTRAIL_MITRE_MAPPING.get(event_name)
        if ct_result:
            return ct_result[0], ct_result[2]
    return "", ""
...
def _build_base_incident(ts, source_ip, hostname, username, finding_type, severity, description, attack_count=1):
    ...
    mitre_technique, mitre_tactic = _lookup_mitre(finding_type)
    return {
        "_id_hash": hashlib.md5(description.encode(), usedforsecurity=False).hexdigest(),
        "id": f"cw-{int(hashlib.md5(description.encode(), usedforsecurity=False).hexdigest()[:12], 16) % 10**12:012d}",
        "incident_id": f"cw-{int(hashlib.md5(description.encode(), usedforsecurity=False).hexdigest()[:8], 16) % 10**8:08d}",
...
# Broad pattern for pipeline status counting (includes non-failure security events)
BROAD_SECURITY_PATTERN = re.compile(
...
def fetch_findings():
    ...
    for lg in log_groups:
        ...
        for fail in failed_attempts:
            ...
            all_failed_incidents.append(_build_base_incident(
                ts=ts,
                source_ip=source_ip,
                hostname=hostname,
                username=username,
            ...
            # Track ALL attempts (undeduplicated) for brute-force rollup
        ...
        for inc in all_failed_incidents:
            ...
            is_dup = False
            if is_pairing and sip and sip != "N/A" and pd.notna(ts_p):
                for existing in incidents:
                    if existing.get("source_ip") != sip:
                        continue
                    if existing.get("finding_type") not in PRIMARY_TYPES:
                        continue
                    if pd.isna(existing.get("timestamp")):
                        continue
                    if abs(existing.get("timestamp") - ts_p) < pd.Timedelta(seconds=2):
                        is_dup = True
                        break
                if not is_dup:
                    for existing in all_failed_incidents:
                        if existing is inc:
                            continue
                        if existing.get("source_ip") != sip:
                            continue
...
def fetch_pipeline_status():
    ...
    for lg in log_groups:
        ...
        for row in rows:
            fields = {f["field"]: f.get("value", "") for f in row}
            msg = fields.get("@message", "")
            if BROAD_SECURITY_PATTERN.search(msg):
